...
export class FakeAuthServer {
    ...
    switch (url.pathname) {
      case "/.well-known/oauth-protected-resource/mcp":
        return json(res, {
          ...
          scopes_supported: ["genie:ask", "genie:self"],
        });
  ...
  private authorize(url: URL, res: ServerResponse): boolean {
    ...
    const registered =
      ...
      q.get("scope") === "genie:ask genie:self" &&
  ...
  private issue(): Record<string, unknown> {
    ...
    return { access_token: access, refresh_token: refresh, token_type: "Bearer", expires_in: 3600, scope: "genie:ask genie:self" };
  }
}
